...
export const validatePackage = (
  ...
  if (documentDesc === "") {
    ...
    if (commodities.includes(documentDesc.toLowerCase())) {
      handleValidationChange("documentDesc_comm_error", true, setIsError)
      isValidDesc = false
    } else {
      handleValidationChange("documentDesc_comm_error", false, setIsError)
      isValidDesc = true
    }
  }
  ...
  if (
    (documentWeight === "" || documentWeight === "0") &&
  ...
  if (upperDetails.detail_type === "document") {
    isValid =
      ...
      isValidCustoms &&
      isValidWeight &&
      isValidCompareWeight
  }

  return isValid
}

export const validateItemDetails = (item, commodities, setIsItemError) => {
  var isValid = true
  var isValidDesc = true
  ...
  if (item.description === "" || item.description === undefined) {
    ...
    if (commodities.includes(item.description.toLowerCase())) {
      handleValidationChange("description_comm_error", true, setIsItemError)
      isValidDesc = false
    } else {
      handleValidationChange("description_comm_error", false, setIsItemError)
  ...
  isValid =
    isValidDesc && isValidMade && isValidQty && isValidWeight && isValidCustoms
